...
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_fast_recall                          | Быстрый перезапуск предыдущей команды. Ускорение достигается благодаря передаче  только части операндов (только ключ, только младшая часть ключа и т.д.)                              |
| **Сервисные функции  Leonhard API**                                  | В группу входят функции преобразования типов и ожидания готовности lnh64                                     |
| &nbsp; &nbsp; &nbsp; &nbsp; float2uint                               | Представление значения типа  float в целочисленном виде (инферсия знака мантиссы)  для сохранения в ввиде поля ключа. Команда позволяет исопльзовать целочиссленное безщнаковое сравнение для чисел float |
| &nbsp; &nbsp; &nbsp; &nbsp; uint2float                               | Функция, обратная к float2uint, позволяет преобразовать часть ключа, сохраненного в  виде unsigned int в тип float |
| &nbsp; &nbsp; &nbsp; &nbsp; double2ull                               | Представление значения типа double в целочисленном виде (инферсия знака мантиссы)  для сохранения в ввиде поля ключа. Команда позволяет исопльзовать целочиссленное безщнаковое сравнение для чисел double |
| &nbsp; &nbsp; &nbsp; &nbsp; ull2double                               | Функция, обратная к double2ull, позволяет преобразовать часть ключа, сохраненного в  виде unsigned int в тип double |
...
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_syncm                                | Ожижание готовности регистра Mailbox. При запуске команд DISC с записью результатов в регистры mbr сбрасывется флаг достоверности указанных регистров. При помещении результатов в  mbrx регистр, флаг устанавливается и разрешается его чтение. |
...
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_next                |   Поиск следующего ключа, следующего за ключом key в структуре str. Поиск выполняется в целочисленном беззнаковом порядке на ключах. |      
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_prev                |   Поиск предыдущего ключа, следующего перед ключом key в структуре str. Поиск выполняется в целочисленном беззнаковом порядке на ключах. |      
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_nsm                 |   Поиск следующего ключа, большего значению key в структуре str (ключ key может отсутствовать в структуре). Поиск выполняется в целочисленном беззнаковом порядке на ключах. |      
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_ngr                 |   Поиск следующего ключа, меньшего значению key в структуре str (ключ key может отсутствовать в структуре). Поиск выполняется в целочисленном беззнаковом порядке на ключах. |      
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_nsm_signed          |   Поиск следующего ключа, большего значению key в структуре str (ключ key может отсутствовать в структуре). Поиск выполняется в целочисленном знаковом порядке на ключах. |      
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_ngr_signed          |   Поиск следующего ключа, меньшего значению key в структуре str (ключ key может отсутствовать в структуре). Поиск выполняется в целочисленном знаковом порядке на ключах. |      
...
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_ins_syncq           |  Вставка ключа key и значения value в структуру str.  Запись результата в очередь.    |                   
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_del_syncq           |  Удаление записи с ключом key из структуры str. Запись результата в очередь.     |               
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_get_numq            |  Получить количество записей в структуре str.Запись результата в очередь.     |                 
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_del_str_syncq       |  Удаление структуры str.  Запись результата в очередь.     |                   
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_sq_syncq            |  Сжатие структуры в памяти lnh64 (сокращение занимаемого объема памяти).  Запись результата в очередь.     |             
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_or_syncq            |  Объединение множеств ключей структуры A и B. Помещение результирующей структуры в R. Запись результата в очередь.     |           
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_and_syncq           |  Пересечение множеств ключей структуры A и B. Помещение результирующей структуры в R. Запись результата в очередь.     |         
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_not_syncq           |  Дополнение множеств ключей структуры A ключами структуры B. Помещение результирующей структуры в R. Запись результата в очередь.     |              
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_lseq_syncq          |  Срез структуры A по условию "меньше или равно" ключа key. Помещение результирующей структуры в R (все ключи, не соответствующие условию lseq в структуре R отсуствуют).  Запись результата в очередь.     |            
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_ls_syncq            |  Срез структуры A по условию "меньше" ключа key. Помещение результирующей структуры в R. (все ключи, не соответствующие условию ls в структуре R отсуствуют). Запись результата в очередь.     |                
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_greq_syncq          |  Срез структуры A по условию "больше или равно" ключа key. Помещение результирующей структуры в R.(все ключи, не соответствующие условию greq в структуре R отсуствуют). Запись результата в очередь.     |                           
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_gr_syncq            |  Срез структуры A по условию "больше" ключа key. Помещение результирующей структуры в R. (все ключи, не соответствующие условию gr в структуре R отсуствуют).  Запись результата в очередь.     |                 
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_grls_syncq          |  Срез структуры A по условию "меньше" ключfа key_ls и "больше" ключа key_gr. Помещение результирующей структуры в R. (все ключи, не соответствующие условию gr и ls в структуре R отсуствуют). Запись результата в очередь.     |              
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_searchq             |  Поиск ключа key в структуре str и выдача найденного ключа и значения value. Запись результата в очередь.     |              
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_nextq               |  Поиск следующего ключа, следующего за ключом key в структуре str. Поиск выполняется  в целочисленном беззнаковом порядке на ключах. Запись результата в очередь.     |           
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_prevq               |  Поиск предыдущего ключа, следующего перед ключом key в структуре str. Поиск выполняется  в целочисленном беззнаковом порядке на ключах.  Запись результата в очередь.     |               
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_nsmq                |  Поиск следующего ключа, большего значению key в структуре str (ключ key может отсутствовать в структуре).  Поиск выполняется в целочисленном беззнаковом порядке на ключах. Запись результата в очередь.     |                
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_ngrq                |  Поиск следующего ключа, меньшего значению key в структуре str (ключ key может отсутствовать в структуре).  Поиск выполняется в целочисленном беззнаковом порядке на ключах. Запись результата в очередь.     |          
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_get_firstq          |  Поиск наименьшего ключа структуры str. Поиск выполняется в целочисленном беззнаковом порядке на ключах. Запись результата в очередь.     |                
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_get_lastq           |  Поиск наибольшего ключа структуры str. Поиск выполняется в целочисленном беззнаковом порядке на ключах. Запись результата в очередь.     |              
...
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_ins_syncm           | Вставка ключа key и значения value в структуру str. Запись результата в регистр st_mreg. |
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_del_syncm           |  Удаление записи с ключом key из структуры str.  Запись результата в регистр st_mreg.  |   
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_get_numm            |  Получить количество записей в структуре str.  Запись результата в регистр mrf.  |      
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_del_str_syncm       |  Удаление структуры str.  Запись результата в регистр st_mreg.  |      
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_sq_syncm            |  Сжатие структуры в памяти lnh64 (сокращение занимаемого объема памяти).  Запись результата в регистр st_mreg.  |     
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_or_syncm            |  Объединение множеств ключей структуры A и B. Помещение результирующей структуры в R.  Запись результата в регистр st_mreg.  |        
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_and_syncm           |  Пересечение множеств ключей структуры A и B. Помещение результирующей структуры в R.  Запись результата в регистр st_mreg.  |      
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_not_syncm           |  Дополнение множеств ключей структуры A ключами структуры B. Помещение результирующей структуры в R.  Запись результата в регистр st_mreg.  |       
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_lseq_syncm          |  Срез структуры A по условию "меньше или равно" ключа key. Помещение результирующей структуры в R (все ключи, не соответствующие условию lseq в структуре R отсуствуют).  Запись результата в регистр st_mreg.  |         
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_ls_syncm            |  Срез структуры A по условию "меньше" ключа key. Помещение результирующей структуры в R. (все ключи, не соответствующие условию ls в структуре R отсуствуют).  Запись результата в регистр st_mreg.  |         
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_greq_syncm          |  Срез структуры A по условию "больше или равно" ключа key. Помещение результирующей структуры в R.(все ключи, не соответствующие условию greq в структуре R отсуствуют).  Запись результата в регистр st_mreg.  |        
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_gr_syncm            |  Срез структуры A по условию "больше" ключа key. Помещение результирующей структуры в R. (все ключи, не соответствующие условию gr в структуре R отсуствуют).  Запись результата в регистр st_mreg.  |          
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_grls_syncm          |  Срез структуры A по условию "меньше" ключfа key_ls и "больше" ключа key_gr. Помещение результирующей структуры в R. (все ключи, не соответствующие условию gr и ls в структуре R отсуствуют).  Запись результата в регистр st_mreg.  |        
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_searchm             |  Поиск ключа key в структуре str и выдача найденного ключа и значения value.  Запись результата в регистр st_mreg.  |         
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_nextm               |  Поиск следующего ключа, следующего за ключом key в структуре str. Поиск выполняется  в целочисленном беззнаковом порядке на ключах.  Запись результата в регистр st_mreg.  |       
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_prevm               |  Поиск предыдущего ключа, следующего перед ключом key в структуре str. Поиск выполняется  в целочисленном беззнаковом порядке на ключах.   Запись результата в регистр st_mreg.  |     
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_nsmm                |  Поиск следующего ключа, большего значению key в структуре str (ключ key может отсутствовать в структуре).  Поиск выполняется в целочисленном беззнаковом порядке на ключах.  Запись результата в регистр st_mreg.  |      
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_ngrm                |  Поиск следующего ключа, меньшего значению key в структуре str (ключ key может отсутствовать в структуре).  Поиск выполняется в целочисленном беззнаковом порядке на ключах.  Запись результата в регистр st_mreg.  |       
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_get_firstm          |  Поиск следующего ключа, большего значению key в структуре str (ключ key может отсутствовать в структуре).  Поиск выполняется в целочисленном знаковом порядке на ключах.   Запись результата в регистр st_mreg.  |       
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_get_lastm           |  Поиск следующего ключа, меньшего значению key в структуре str (ключ key может отсутствовать в структуре).  Поиск выполняется в целочисленном знаковом порядке на ключах.  Запись результата в регистр st_mreg.  |       
| &nbsp; &nbsp; &nbsp; &nbsp; lnh_get_m               |  Чтение регистра Mailbox   Запись результата в регистр st_mreg.  |   
