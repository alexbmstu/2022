...
```
Сформировать в хост-подсистеме и передать в SPE таблицу маршрутизации из 510 ip адресов 195.19.32.1/23. Каждому адресу поставить в соответствие один из 128 интерфесов (0..127). Выполнить тестирование работы, посылая их хост-подсистемы ip адреса и сравнивая полученный от GPC мером маршрута с ожидаемым.  
```

**Вариант 2**

```
Сформировать в хост-подсистеме и передать в SPE 1024 записи key-value со значениями функции f(x)=sin(2*x). Выполнить тестирование работы структуры, посылая из хост-подсистемы значение x и получая от GPC значение f(x). Если указанного значения x не сохранено в SPE, выполнить поиск ближайшего (меньшего или большего) значени к точке x и вернуть f(x). Сравнить результат с ожидаемым.  
```

**Вариант 3**

```
Сформировать в хост-подсистеме и передать в SPE 1024 записи множества A (случайные числа 0..1048576) и 1024 записи мнжества B (числа от 0..1048576). Сформировать в SPE множество C = A and B. Выполнить тестирование работы SPE, сравнив набор ключей в множестве C с ожидаемым.   
```
