...
> **Сетевой маршрутизатор на 128 портов**. Сформировать в хост-подсистеме и передать в SPE таблицу маршрутизации из 254 ip адресов 195.19.32.1/24 (адреса 195.19.32.1 .. 195.19.32.254). Каждому адресу поставить в соответствие один из 128 интерфесов (целые числа 0..127). Выполнить тестирование работы маршрутизатора, посылая из хост-подсистемы ip адреса и сравнивая полученный от GPC номер маршрута с ожидаемым.  
...
> **Маршрутизатор с QoS**. Сформировать в хост-подсистеме и передать в SPE таблицу маршрутизации из 32 ip адресов 195.19.32.1/24 (адреса 195.19.32.1 .. 195.19.32.32), где для каждого адреса доступны 8 вариантов маршрутизации (всего 256 записей в таблице). Каждому адресу поставить в соответствие восемь вариантов из 128 интерфесов (целые числа 0..127). Вариант определяется по номеру параметра QoS, принимающему значения от 0 до 7. Выполнить тестирование работы маршрутизатора, посылая из хост-подсистемы уровень QoS и ip адреса и сравнивая полученный от GPC номер маршрута с ожидаемым.  
...
> **Устройство управления записью в SSD накопитель**. Сформировать в SPE таблицу из 1048576 записей с номерами свободных страниц и количеством перезаписей в них (в начальный момент таблица содержит все записи, количество перезаписей равно 0) и вторую таблицу из 1048576 занятых страниц и количеством перезаписей в них (в начальный момент таблица пуста). Для таблиц старшей частью ключа является количество перезаписей, а младшей частью ключа номер страницы. При поступлении от хост-подсистемы запроса на запись удалить из таблицы свободных страниц запись с минимальным количеством перезаписей, увеличить количество перезаписей, и эту информацию в таблицу занятых страниц. Вернуть в хост подсистему номер записанной страницы и количество перезаписей. При поступлении от хост системы запроса на освобождение страницы произвести обратное действие, количество перезаписей оставить неизменным.
