...
Для многих социальных сетей, например для сети актеров кино, необходимо определить относительную важность входящих в нее узлов. Например, загруженность актера i в сети актеров (между двумя актерами кино имеется связь, если они снялись в одном фильме) определяется как суммарное число кратчайших путей между всеми остальными актерами (узлами), которые проходят через актера (узел) i:

<img src="https://latex.codecogs.com/svg.image?B(i)&space;=\sum_{st}^{}\frac{\sigma_{st}(i)}{\sigma_{st}}">

где:

σ<sub>st</sub>(i) – число кратчайших путей из узла s в узел t через узел i;

σ<sub>st</sub>  – общее число кратчайших путей между всеми парами s и t.

Эту величину можно считать индикатором наиболее влиятельных и важных персон (VIP) в социальной сети или белковое соединение, участвующее в наибольшем количестве реакций. Эта величина также важна в изучении транспортных потоков и обычно называется нагрузкой (загруженностью) узла (или связи), поскольку характеризует долю проходящих через узел кратчайших путей и узлы с высоким значением B являются наиболее загруженными. В отличие от степени узла (количества ребер, инцидентных вершине), понятие важности узла отражает топологию всей сети.
...
Центральность()
	...
	ЦИКЛ ПОКА  
		Для всех u ∈ V
			...
			ЦИКЛ ПОКА 
				Для всех v ∈ V , v /= s 
				ЕСЛИ кратчайший путь проходит через v
					btwc[v] = btwc[v] + 1;
				ВСЕ ЕСЛИ
			ВСЕ ЦИКЛ	
	ВСЕ ЦИКЛ ПОКА
...
![Пример поиска и визуализации свойства центральности на графе решетки](assets/btwc.jpg)
