...
> Сформировать в хост-подсистеме и передать в SPE 1024 записи key-value со значениями функции f(x)=sin(2 * x) в диапазоне 0 до 2 * Pi. Выполнить тестирование работы структуры, посылая из хост-подсистемы значение x и получая от GPC значение f(x). Если указанного значения x не сохранено в SPE, выполнить поиск ближайшего (меньшего или большего) значени к точке x и вернуть f(x). Сравнить результат с ожидаемым.  
