...
> Сформировать в хост-подсистеме и передать в SPE таблицу маршрутизации из 510 ip адресов 195.19.32.1/23. Каждому адресу поставить в соответствие один из 128 интерфесов (0..127). Выполнить тестирование работы, посылая их хост-подсистемы ip адреса и сравнивая полученный от GPC номер маршрута с ожидаемым.  


**Вариант 2**

> Сформировать в хост-подсистеме и передать в SPE 1024 записи key-value со значениями функции f(x)=x^2 в диапазоне 0 до 1048576. Выполнить тестирование работы структуры, посылая из хост-подсистемы значение x и получая от GPC значение f(x). Если указанного значения x не сохранено в SPE, выполнить поиск ближайшего (меньшего или большего) значени к точке x и вернуть f(x). Сравнить результат с ожидаемым.  


**Вариант 3**

> Сформировать в хост-подсистеме и передать в SPE 1024 записи множества A (случайные числа в диапазое 0..16383) и 1024 записи мнжества B (случайные числа в диапазоне 0..16383). Сформировать в SPE множество C = A and B. Выполнить тестирование работы SPE, сравнив набор ключей в множестве C с ожидаемым.   


**Вариант 4**

> Сформировать в хост-подсистеме и передать в SPE 1024 числа с плавающей запятой типа double. Для случайного значения, сформированного в хост-подсистеме выполнить поиск ближайшего большего в SPE. Выполнить тестирование работы SPE, сравнив результат с ожидаемым.  Для представления числа double в целочисленном беззнаковом диапазоне использовать функции double2ull и ull2double. 
