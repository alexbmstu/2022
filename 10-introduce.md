...
Граф – множество вершин *X*, на элементах которого определены двуместные отношения смежности (ребра) – *(x<sub>j</sub>, x<sub>i</sub>) inF* , где *x<sub>j</sub>*,*x<sub>i</sub> in X*. Тогда пара вершин, находящихся в отношении смежности, рассматривается как ребро *u<sub>k</sub> = (x<sub>j</sub>, x<sub>i</sub>), u<sub>k</sub> in U*.
