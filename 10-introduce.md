...
* Хост-подсистема передает сообщение с количеством ключей и значений (mq_send(BURST)).

* Программное ядро в обработчике insert_burst получает сообщение и выделяет буфер (buffer) для ранения данных в RAM GPC.
...
> **Сетевой маршрутизатор на 128 портов**. Сформировать в хост-подсистеме и передать в SPE таблицу маршрутизации из 254 ip адресов 195.19.32.1/24 (адреса 195.19.32.1 .. 195.19.32.254). Каждому адресу поставить в соответствие один из 128 интерфесов (целые числа 0..127). Выполнить тестирование работы маршрутизатора, посылая их хост-подсистемы ip адреса и сравнивая полученный от GPC номер маршрута с ожидаемым.  

**Вариант 2**

> **Цифровой интерполятор**. Сформировать в хост-подсистеме и передать в SPE 256 записи key-value со значениями функции `f(x)=x^2` в диапазоне значений `x` от 0 до 1048576 (где x - ключ, f(x) - значение). Выполнить тестирование работы устройства, посылая из хост-подсистемы значение `x` и получая от sw_kernel значение `f(x)`. Если указанного значения `x` не сохранено в SPE, выполнить поиск ближайшего (меньшего или большего) значения к точке `x` и вернуть соответствующий `f(x)`. Сравнить результат с ожидаемым.  
...
> **Устройство интегрирования**. Сформировать в хост-подсистеме и передать в SPE 256 записи с ключами `x` и значениями `f(x)=x^2` в диапазоне значений `x` от 0 до 1048576. Передать в sw_kernel числа `x1` и `x2` (`x2>x1`). В хост-подсистему вернуть сумму значений `f(x)` на диапазоне (`x1,x2`). Сравнить результат с ожидаемым.  

**Вариант 7**

> **Блок пакетной передачи**. Сформировать в хост-подсистеме буфер из 1048576 ключей и значений (по 64 бит). Передать буфер через глобальную память в SPE блоками по 4096 байт и выполнить вставку ключей и значений в SPE. Выполнить обход полученной структуры и результат передать в хост-подсистему блоками по 4096 байт. Сравнить результат с ожидаемым.  

**Вариант 8**

> **Устройство вычисления обратной функции**.  Сформировать в хост-подсистеме и передать в SPE 256 записи key-value со значениями функции `f(x)=x^2` в диапазоне значений `x` от 0 до 1048576 (где f(x) - ключ, x - значение). Выполнить тестирование работы устройства, посылая из хост-подсистемы значение `f(x)` и получая от sw_kernel значение `x`. Если указанного значения `f(x)` не сохранено в SPE, выполнить поиск ближайшего (меньшего или большего) значения к `f(x)` и вернуть соответствующий `x`. Сравнить результат с ожидаемым. 
...
> **Маршрутизатор с QoS**. Сформировать в хост-подсистеме и передать в SPE таблицу маршрутизации из 32 ip адресов 195.19.32.1/24 (адреса 195.19.32.1 .. 195.19.32.32), где для каждого адреса доступны 8 вариантов маршрутизации (всего 256 записей в таблице). Каждому адресу поставить в соответствие восемь вариантов из 128 интерфесов (целые числа 0..127). Вариант определяется по номеру параметра QoS, принимающему значения от 0 до 7. Выполнить тестирование работы маршрутизатора, посылая их хост-подсистемы уровень QoS и ip адреса и сравнивая полученный от GPC номер маршрута с ожидаемым.  
...
### 3.2.1. Представление структур данных в виде ключей и значений <a name="3_2_1"></a>
